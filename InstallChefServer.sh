...
# Trap arguments
while getopts a:c:v:u:p:n:e:s:f:j:jv:m:mv:pv: option
do
 case "${option}"
 in
 ...
 n) chef_server_user_fullname=${OPTARG};;
 ...
 jv) chef_server_pushjobs_version=${OPTARG};;
 m) chef_server_install_manage=${OPTARG};;
 mv) chef_server_manage_version=${OPTARG};;
...
if [ ! $(sudo chef-server-ctl user-list | grep $chef_server_user) ]; then
  ...
  chef-server-ctl user-create $chef_server_user $chef_server_user_fullname $chef_server_user_email $chef_server_user_password --filename /drop/$chef_server_user.pem
